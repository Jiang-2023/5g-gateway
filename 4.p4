...
control MyIngress(inout headers hdr,
    ...
    action convert_to_5G_format() { // 转换为5G GTP格式
        hdr.gtp.flags = 0x30;
        hdr.gtp.messageType = 0xFF;  // 用户定义的类型
        hdr.gtp.length = hdr.ipv4.totalLen + 8;  // GTP header 长度加上IPv4数据长度
        hdr.gtp.teid = 0x000000000;  // 随机生成的TEID，实际场景应根据需求生成
}
...
control MyDeparser(packet_out packet, in headers hdr) {
    apply {
        ...
        packet.emit(hdr.gtp);           // 输出GTP头部，用于5G传输
    }
}
